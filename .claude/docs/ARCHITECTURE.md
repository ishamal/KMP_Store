...
dirs:
  - core/<name>/{api,real}: api = interfaces + pure types; real = implementations + Compose UI + Metro bindings. Modules: di, model, experience, session, navigation, ui.
    ...
    - core/experience: ExperienceSnapshot (access model) + ExperienceProvider (pure builder — computes a snapshot from experience/BU/role/capability inputs; no longer holds the live one). The live snapshot is bound non-null inside ExperienceGraph (@GraphExtension(ExperienceScope::class), core/experience/real), recreated per BU/experience switch and dropped at logout. Snapshot-consuming ViewModels contribute into ExperienceScope and receive non-null ExperienceSnapshot at construction.
